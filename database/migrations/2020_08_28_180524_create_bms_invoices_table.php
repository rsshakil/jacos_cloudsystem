...
class CreateBmsInvoicesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('bms_invoices', function (Blueprint $table) {
            ...
            $table->string('sta_doc_instanceId', 10)->comment('インスタンスＩＤ');
            $table->string('sta_doc_type', 10)->comment('メッセージ種');
            $table->string('sta_doc_creationDateAndTime', 10)->comment('作成日時');
            $table->string('sta_bus_scope_instanceIdentifier', 10)->comment('テスト区分ＩＤ');
            $table->string('sta_bus_scope_identifier', 10)->comment('最終送信先ＩＤ');
            $table->string('mes_ent_uniqueCreatorIdentification', 10)->comment('メッセージ識別ＩＤ');
            $table->string('mes_mes_senderStationAddress', 10)->comment('送信者ステーションアドレス');
            $table->string('mes_mes_ultimateReceiverStationAddress', 10)->comment('最終受信者ステーションアドレス');
            $table->string('mes_mes_immediateReceiverStationAddres', 10)->comment('直接受信者ステーションアドレス');
            $table->string('mes_mes_numberOfTradingDocuments', 10)->comment('取引数');
            $table->string('mes_mes_systemInfo', 10)->comment('システム情報');
            ...
            $table->string('mes_lis_inv_per_beginDate', 10)->comment('対象期間開始');
            $table->string('mes_lis_inv_per_endDate', 10)->comment('対象期間終了');
            $table->string('mes_lis_inv_lin_lin_tradeNumberReference', 10)->comment('取引番号（発注・返品）');
            $table->string('mes_lis_inv_lin_lin_issueClassificationC', 10)->comment('発行区分');
            $table->string('mes_lis_inv_lin_lin_sequenceNumber', 10)->comment('連番');
            ...
            $table->string('mes_lis_inv_lin_det_goo_majorCategory', 10)->comment('商品分類（大）');
            $table->string('mes_lis_inv_lin_det_goo_subMajorCategory', 10)->comment('商品分類（中）');
            $table->string('mes_lis_inv_lin_det_transferOfOwnershipD', 10)->comment('計上日');
            $table->string('mes_lis_inv_lin_det_amo_requestedAmount', 10)->comment('請求金額');
            $table->string('mes_lis_inv_lin_det_amo_req_plusMinus', 10)->comment('請求金額符号');
            $table->string('mes_lis_inv_lin_det_amo_tax', 10)->comment('税額合計金額');
            $table->string('mes_lis_inv_lin_det_balanceCarriedCode', 10)->comment('請求区分');
            $table->string('mes_lis_inv_lin_det_creditOrUnsettlement', 10)->comment('未払買掛区分');
            $table->string('mes_lis_inv_lin_det_payCode', 10)->comment('支払内容');
            $table->string('mes_lis_inv_lin_det_tax_taxTypeCode', 10)->comment('税区分');
            $table->string('mes_lis_inv_lin_det_tax_taxRate', 10)->comment('税率');
