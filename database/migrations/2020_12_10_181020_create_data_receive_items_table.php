...
class CreateDataReceiveItemsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('data_receive_items', function (Blueprint $table) {
            ...
            $table->string('mes_lis_acc_lin_fre_shipment_item_code',10)->default('')->comment('商品コード（出荷元）');
            ...
            $table->string('mes_lis_acc_lin_qua_shi_quantity',20)->default('')->comment('出荷数量（バラ）');
            $table->string('mes_lis_acc_lin_qua_shi_num_of_order_units',20)->default('')->comment('出荷数量（発注単位数）');
            $table->string('mes_lis_acc_lin_qua_rec_quantity',20)->default('')->comment('受領数量（バラ）');
            $table->string('mes_lis_acc_lin_qua_rec_num_of_order_units',20)->default('')->comment('受領数量（発注単位数）');
            $table->string('mes_lis_acc_lin_qua_rec_reason_code',20)->default('')->comment('訂正区分');
